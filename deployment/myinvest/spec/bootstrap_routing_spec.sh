...
grep -Fq "website_webhook_url = key == 'saas' ? ENV.fetch('MYINVEST_REBOOKING_WEBHOOK_URL') : nil" "$seed"
[[ "$(grep -Fc 'webhook_url: website_webhook_url' "$seed")" -eq 2 ]]
grep -Fq "channel_updates[:webhook_url] = website_webhook_url if key == 'saas'" "$seed"
! grep -Fq 'webhook_url: nil' "$seed"
...
grep -Fq 'dashboard\?support_history=1' "$smoke"
grep -Fq 'dashboard\?support_history=1' "$deployment_dir/scripts/validate.sh"
grep -Fq 'INTERN_SSO_RETURN_PATH: ${INTERN_SSO_RETURN_PATH:-/app/accounts/1/dashboard?support_history=1}' "$deployment_dir/compose.yaml"
