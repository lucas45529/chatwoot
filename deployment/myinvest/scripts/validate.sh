...
required=(
  ...
  ACADEMY_NEW_WEBSITE_URL ACADEMY_LEGACY_WEBSITE_URL MYINVEST_REBOOKING_WEBHOOK_URL
)
...
if [[ "$LOCAL_SMOKE" != true ]]; then
  ...
  production_required=(
    ...
    STORAGE_REGION STORAGE_ENDPOINT BACKUP_GPG_RECIPIENT BACKUP_GPG_SIGNING_KEY
  ...
  (( ${#PSEUDONYMIZATION_KEY} >= 32 )) || {
    printf 'PSEUDONYMIZATION_KEY must be at least 32 characters.\n' >&2
    exit 1
  }
  [[ "$PSEUDONYMIZATION_KEY" != "$SUPPORT_ANSWER_SECRET" ]] || {
    printf 'PSEUDONYMIZATION_KEY must be independent from SUPPORT_ANSWER_SECRET.\n' >&2
    exit 1
  }
...
if [[ "$AGENT_LEARNING_DATABASE_USER" == "$POSTGRES_ADMIN_USER" ||
      "$AGENT_LEARNING_DATABASE_USER" == "$POSTGRES_USERNAME" ||
      "$AGENT_LEARNING_DATABASE_USER" == "$CLAUDE_AGENT_DATABASE_USER" ]]; then
  printf 'Agent learning database user must be a distinct read-only identity.\n' >&2
  exit 1
fi

expected_agent_learning_url="postgresql://${AGENT_LEARNING_DATABASE_USER}:${AGENT_LEARNING_DATABASE_PASSWORD}@postgres:5432/${POSTGRES_DATABASE}"
if [[ "$AGENT_LEARNING_CHATWOOT_DATABASE_URL" != "$expected_agent_learning_url" ]]; then
  printf 'AGENT_LEARNING_CHATWOOT_DATABASE_URL must use the dedicated read-only identity.\n' >&2
  exit 1
fi
...
rails|myinvest-chatwoot:4.16.2-e868e4d
...
for helper in "$deployment_dir/scripts/"*.sh; do
  bash -n "$helper"
done
