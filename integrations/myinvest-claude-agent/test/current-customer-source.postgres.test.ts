...
it.skipIf(!process.env.LEARNING_TEST_DATABASE_URL)('proves live source identity and rejects newer public incoming/human messages in PostgreSQL', async () => {
  ...
  try {
    ...
    await client.query(`CREATE TEMP TABLE attachments (id bigint, account_id bigint, message_id bigint, file_type integer)`)
    await client.query(`CREATE TEMP TABLE active_storage_attachments (record_type text, record_id bigint, name text, blob_id bigint)`)
    await client.query(`CREATE TEMP TABLE active_storage_blobs (id bigint, byte_size bigint)`)
    ...
    await client.query(`INSERT INTO attachments VALUES (29,101,55,1)`)
    await client.query(`INSERT INTO active_storage_blobs VALUES (129,1800)`)
    await client.query(`INSERT INTO active_storage_attachments VALUES ('Attachment',29,'file',129)`)
    expect(await store.loadCurrentSource(input)).toMatchObject({ hasAudioAttachment: true, audioAttachment: { id: 29, byteSize: 1800 } })
    await client.query(`INSERT INTO attachments VALUES (30,101,55,1)`)
    await client.query(`INSERT INTO active_storage_blobs VALUES (130,1800)`)
    await client.query(`INSERT INTO active_storage_attachments VALUES ('Attachment',30,'file',130)`)
    expect((await store.loadCurrentSource(input))?.hasAudioAttachment).toBe(true)
    expect((await store.loadCurrentSource(input))?.audioAttachment).toBeUndefined()
    await client.query(`DELETE FROM attachments WHERE id=30`)
